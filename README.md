...
- **v1.2** — если Polling D4 4A вернул 0 целей (карта уже активирована
  PC/SC-слоем, чип PN532 не отдаёт её повторно), тип карты определяется по
  синтетическому ATR ридера (байты H10-H11, коды ACS) — и чтение секторов
  Mifare продолжается; исправлено ложное «ICC внутри PICC» (интерфейсы
  различаются по границе слова `\bICC\b`); PPSE/EMV уходят только на
  реальный контактный интерфейс. В отчёте появляются «Код карты (ACS)»
  и «Источник типа».
...
```
acr1281_dump.py    основной скрипт (консоль, v1.1)
acr1281_gui.py     графический интерфейс (tkinter), использует логику acr1281_dump.py
requirements.txt   зависимости (pyscard)
run.bat            запуск консольной версии двойным кликом
...
src/  index.html   сайт-справка: код, разбор ATR, образцы отчёта, демо GUI, установка
```

Оба `.py`-файла должны лежать в одной папке: GUI импортирует общую логику
чтения из консольной версии.
...
4. Запуск:
   ...
   ```powershell
   python acr1281_dump.py     # консольная версия
   python acr1281_gui.py      # или графический интерфейс (tkinter)
   ```

В папке появятся `card_report_<UID>_<ГГГГММДД_ЧЧММСС>.txt` и `.json`
(в GUI — через диалог «Сохранить отчёт…», содержимое то же).
